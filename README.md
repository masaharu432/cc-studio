...
| webview（Markdown プレビュー / Claude Code 拡張チャット）が左右の広い余白で細い帯に閉じ込められ、横幅を使えない | `view-width` プラグイン — 無駄な左右余白を詰めて全幅表示。プレビュー（本文の左右余白）とチャット（会話列の中央寄せ＋メッセージ左右余白）を `previewGutter` / `chatGutter` として ⚙ の −/+ ステッパーで**個別にライブ調整**（リロード不要・「デフォルトに戻す」付き。**0 で最大幅**、既定はプレビュー 12px・チャット 0） |
