...
| webview（Markdown プレビュー / Claude Code 拡張チャット）が左右の広い余白で細い帯に閉じ込められ、横幅を使えない | `view-width` プラグイン — 無駄な左右余白を詰めて全幅表示。**プレビュー本文**（`previewGutter`）・チャットの**メッセージ**（`chatGutter`）・チャットの**入力欄**（`inputGutter`）を ⚙ の −/+ ステッパーで**それぞれ独立にライブ調整**（リロード不要・「デフォルトに戻す」付き。**0 で最大幅**、既定はプレビュー 12px・メッセージ/入力欄 0） |
