...
| webview（Markdown プレビュー / Claude Code 拡張チャット）が左右の広い余白で細い帯に閉じ込められ、横幅を使えない | `view-width` プラグイン — 無駄な左右余白を詰めて全幅表示。**プレビュー本文**（`previewGutter`）・チャットの**メッセージ**（`chatGutter`）・チャットの**入力欄**（`inputGutter`）を ⚙ の −/+ ステッパーで**それぞれ独立にライブ調整**（リロード不要・「デフォルトに戻す」付き。**0 で最大幅**、既定はプレビュー 8px・メッセージ4px・入力欄4px） |
...
- **Plugins（プラグイン）** — モバイルの摩擦を潰す `.js` を全画面の管理スクリーンで ON/OFF・追加・削除。
  同梱プラグインは [`plugins/`](plugins/) に 12 本（上表のほか、不具合調査用の `focus-hud` / `select-diag`）。
