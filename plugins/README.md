...
| `CCStudio.pluginPublish(topic, payload)` / `pluginSubscribe(topic)` | **汎用プラグイン・メッセージバス（publish/subscribe）**。top→iframe 方向の postMessage が届かない構成での配達路。subscribe はポーリング型（呼ぶたびに次の未消費メッセージを 1 件返す・消費。無ければ空文字）。トピック別 FIFO・30 秒失効・スクリーン別。top に描いた操作 UI のイベントをフレームへ届ける等、どのプラグインからでも使ってよい | rc-indicator |
