...
- extension.js の改変。
- ~~ネイティブ（Kotlin）側の変更~~ → **v0.8 で撤回**: top→iframe の postMessage が全方式で
  届かない実測を受け、CcBridge に RC トグル・メールボックス 2 メソッドを追加（§4）。
- デスクトップ VS Code ネイティブ拡張単体対応。
- バナー以外の RC 関連表示（接続エラー等）の加工。

## 4. アーキテクチャ

> **v0.13 確定**: フレーム内完結へ回帰。検知・ボタン描画・タップ・送信のすべてが chat フレーム内で
> 完結し、フレーム間通信は diag の HUD ログ中継のみ。v0.7〜0.12 のハイブリッド（top 描画＋バス配達）
> は配達自体は成立したが、**可視判定・状態同期の信頼性が確保できず断念**（フレームの自己申告 vis は
> 退避 webview で誤答し、top 側の実表示判定も裏フレームの状態漏れを完全には防げなかった）。
> フレーム内完結ならフレームが隠れればボタンも消えるため「表示中セッションの状態だけが見える」を
> 構造的に満たす。ボタンはチャットパネル**左下（composer 直上）の 44×44 丸ボタン**
> （モック 3 案 `mockups/2026-07-22-rc-pill-placement.png` からユーザー選定: 左下×丸）。
> トグルはタップ発火。以下 v0.7〜0.12 の記録は経緯として残す。

### 旧構成の変遷（v0.7: ハイブリッド＝top 描画＋ホップ中継配達）

```
@all-frames true × @run-at document-start で全フレーム注入
  chat フレーム（composer + sendButton を併せ持つフレーム）:
    バナー検知（MutationObserver + ポーリング）
      ├ hideBanner=ON → display:none（目印属性を付与、OFF でその場復元）
      └ 存在有無 = RC 状態 → postMessage で top へ報告（フレーム ID / 可視フラグつき・実証済み経路）
    トグル依頼の待ち受け → ガード確認 → /remote-control 挿入・送信（フレーム内完結・v0.5 実証済み）
  全フレーム:
    トグル依頼のホップ・バイ・ホップ中継: 直上の親からの依頼だけを受け、
    直下の子 window.frames[i]（同一コンテキスト参照）へ再伝搬
  top フレーム:
    状態レジストリ集約（可視フレーム優先）→ ⋮ ボタン直上・同じ列に「R」ピルを描画
    長押し（600ms フィルゲージ）完了 → 直下の子へトグル依頼を投下（先はホップ中継が届ける）
    HUD 診断ログの中継
```

- **配達方式の変遷（重要な教訓）**:
  - v0.2〜0.4「top からクロスオリジン WindowProxy を深掘り列挙して全子孫へ直接投げる」
    「状態報告の e.source へ直接返信」→ **実機で届かず**（fire sent>0 なのに受信ゼロ）。
  - v0.5 でフレーム内完結（ピルも chat フレーム内）へ改め、タップ→送信の機能を実証。
    ただしピル位置がチャットパネル左端になり、⋮ ボタンの列に置けない不満が残った。
  - v0.7 で**設定ランタイムと同じホップ・バイ・ホップ中継**を試行 → **これも実機で届かず**
    （tap fire は出るが toggle recv ゼロ）。この WebView 構成では top→iframe 方向の
    postMessage 自体が機能しないと結論。
  - v0.8 で**ネイティブブリッジのメールボックス**を採用（設計 §3 の「ネイティブ変更なし」
    非ゴールを撤回する判断）。`addJavascriptInterface` のオブジェクトは**全フレーム
    （クロスオリジン iframe 含む）に注入される**ため、これが唯一の確実な下り経路。
  - v0.9 で RC 専用メソッドをやめ、**汎用プラグイン・メッセージバス（publish/subscribe）**に一般化
    （今回のためだけの口を作らず、今後のプラグインが top に操作 UI を足す場合にも
    アプリ変更なしで使い回せるようにするユーザー方針）:
    - `CCStudio.pluginPublish(topic, payload)` — トピック宛てに発行（どのフレームからでも）
    - `CCStudio.pluginSubscribe(topic)` — ポーリング購読。呼ぶたびに次の未消費メッセージを
...
4. 容器の認定条件（v0.4 で厳格化。実機で会話履歴内の "Remote Control is active…" **システム転記**
   — data-testid を持たず 2 の除外をすり抜ける — に誤ヒットし、RC オフでも緑になった実害への対処）:
   - **テキスト長 ≤300 文字**（実バナーは 1 行 ~80 字。トランスクリプト全域を巻き込んだ容器は数千字）
   - **button 要素を内包**（実バナーは × ボタンを持つ。転記はリンクだけでボタンが無い）
5. 認定した容器に目印属性 `data-cc-ri-banner` を付与。hideBanner=ON なら `style.display='none'`。
   OFF への切替時は目印属性の要素の display を復元する。**認定済み要素も毎 tick 再検証**し、
   条件を外れたら隠しを解除して認定を剥がす（誤認定の固着防止）。
6. **chat 本体フレームの判定**は composer だけでは不十分（フォールバックセレクタ
   `[role="textbox"][aria-multiline="true"]` は VS Code エディタ Monaco にもマッチ）。
   webview 固有の送信ボタン `button[class*="sendButton"]` の存在も要求する。
...
> **経緯**: 配達経路の切り分けのため v0.3〜0.10 の間はタップ発火で検証を重ね、v0.10（汎用バス配達）で
> 「完璧に動作・切替もスムース」を実機確認。v0.11 で本節の長押しへ復帰（確定版）。
> 長押し不発の既知原因は 2 つとも対策済み: 「R」テキストの文字選択→SVG グリフ化、
> 幅 30px での指の揺れによる pointerleave 誤キャンセル→ポインターキャプチャ＋leave キャンセル廃止。
...
- **送信ガード**（すべて満たす場合のみ送信、不成立なら理由を top へ返しピルを短く明滅させて拒否を伝える）:
  1. 処理中でない: `button[class*="sendButton"] [class*="stopIcon"]` が**不在**
     （処理中は送信ボタンが停止ボタンに化けるため、クリックすると応答を中断してしまう。
     state-observer の busy 判定と同一セレクタ）。
  2. 直近 3 秒以内にトグル送信していない（連打による二重トグル防止のデバウンス）。
- **下書きは拒否せず退避→復元**（v0.11.2 で方針変更。長いセッションは下書きが残りがちで、
  拒否だとトグル不能の温床になるため）: 全選択→コマンドで置換して送信し、送信後に composer が
  空になったのを確認してから書き戻す。空でない場合（送信失敗等）は継ぎ足しで壊さないよう
  復元を見送りログに残す。テキスト以外の添付は復元対象外（既知の制約）。
