...
- **配達方式の変遷（重要な教訓）**:
  ...
  - v0.7 で**設定ランタイムと同じホップ・バイ・ホップ中継**を採用（ScreenFactory の
    `__ccApplyPluginSetting` が「evaluateJavascript はメインフレームにしか届かない」問題を
    解決した、実機実績のある方式）。各フレームが**自分の同一コンテキストの** `window.frames[i]`
    へだけ投げるため、クロスオリジン深掘り列挙の罠を踏まない。受信は
    `ev.source === window.parent` の直上親限定（設定ランタイムと同じガード）。
- chat フレーム側の実行コード（ガード・送信）は v0.5 実証済みのものを不変で使用。
  万一中継が届かない場合の切り戻し先は v0.6（フレーム内ピル）。
