...
| `MainActivity` 新規スクリーン | `createWebScreen(TARGET_URL)` | `origin + "/"` |
...
- **origin 変更保存**: `ScreenStore` をクリア → 新オリジンで Web スクリーンを 1 枚作り直し → `KeepAliveService` を再起動（新ホストへ再購読）→ トースト「接続先を更新しました」。
