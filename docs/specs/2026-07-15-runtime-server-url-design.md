...
**設定リストへのカード追加（2 エントリに分離）**: `PanelJson.settingsList` に **2 つ**のエントリを追加する。接続先（ホスト）と初期フォルダは保存の意味・影響範囲が全く違う（前者はサーバそのものの変更＝重い／後者は次の新規スクリーンの開始位置だけ＝軽い）ため、1 ページ・1 保存に混ぜない。
- `server`（🖥️「接続先」）: サブは現在ホスト（authority）、未設定時「未設定 — タップして設定」。
- `defaultfolder`（📁「最初に開くフォルダ」）: サブは現在の初期フォルダ、未設定時「サーバの既定（ホーム）」。

両エントリとも同一アセット `server.html` を **モード切替**（`getServerMode()` が `"host"`/`"folder"` を返す）で開き、片方のセクションだけ表示・それぞれ独立した保存ボタンを出す。`openSettingsEntry("server")`→host、`"defaultfolder"`→folder。

詳細ページ構成（host モード）:

1. **WORKBENCH ORIGIN**: `https://` は入力欄の**上のラベル行**（🔒 HTTPS 固定 · https:// / DOMAIN ONLY）に逃がし、ホスト入力欄を**フル幅**にする。プレフィックスを同一行に置くと tailnet の長いホスト名が見切れるため。`oninput` で検証 3 行（HTTPS / ドメイン / IP 不可）をリアルタイム更新。全 pass 時のみ「接続先を保存」活性。folder モードでは初期フォルダ入力＋参照ピッカーのみを出し、「フォルダを保存」で確定（どんなパスでも保存可）。
...
- **接続先（host）保存**: 検証・保存し `KeepAliveService` を再起動（新ホストへ再購読）。ホストが**変わっていない**ときは「保存するだけ」（画面遷移なし・トーストのみ）。ホストが**実際に変わった**とき（旧 origin と異なり、かつ既存 Web スクリーンがある）だけ、既存スクリーンは旧ホストを指して無効になるため**確認ダイアログ**を出す:
  - 「新ホストで開き直す」→ 旧ホストの Web スクリーンを閉じ、新ホストで初期フォルダのスクリーンを 1 枚開いて表示（切替はユーザー確認後のみ）。
  - 「設定だけ保存」→ 既存スクリーンはそのまま（旧ホストのまま。リロード/新規スクリーンで新ホストに切替）。
  破壊的な全消し・作り直しを保存のたびに勝手に行うことはしない。
- **初期フォルダ（folder）保存**: `server.json` に原子的書き込みするだけ。次の新規スクリーン（および初期スクリーン）から反映。既存スクリーンには触れない。パスはホスト非依存なので origin 変更時も保持。
