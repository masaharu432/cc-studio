...
- **幅の制約源**: 会話コンテンツ列 `[class*=inputWrapper]` が `max-width:680px` ＋ auto マージンで中央寄せ
  ...
  で拾う（バージョン差に強い）。
- **上書き（3系統）**: chat の余白は実測で 3 箇所。① 会話列 `[class*=inputWrapper]` の max-width:680px（広幅の中央帯化）
  → `max-width:none`。② メッセージ列 `[class*=messagesContainer]` の左右 padding 20px（狭幅で常時）→ padding を gutter に。
  ③ 入力欄の親 `[class*=inputContainer]` は **position:absolute** で left/right に ~16px インセット（入力欄がメッセージより
  狭く見える主因）→ 絶対配置なので `left/right` を gutter に・`width:auto`。chatGutter=0 で全幅、>0 は各側 gutter（vw 上限
  `min(px,12vw)` で狭幅でも非崩壊）。実測: 狭幅 gutter=0 で入力欄 161→193（メッセージと一致）、広幅も全幅、gutter=40 で
  左右 40px 中央寄せ・はみ出しなし。

### 設定・検証

- 設定は **部位ごとに独立**: `@setting previewGutter number 12 0 80 4`（プレビュー本文）/ `@setting chatGutter number 0 0 200 4`（チャットのメッセージ列）/ `@setting inputGutter number 0 0 200 4`（チャットの入力欄）
