...
- 設定は **部位ごとに独立**: `@setting previewGutter number 12 0 80 4`（プレビュー本文）/ `@setting chatGutter number 0 0 200 8`（チャットのメッセージ列）/ `@setting inputGutter number 0 0 200 8`（チャットの入力欄）
